...
|---|---|---|---|---|---|
| P0 | Resolve PR #2 integrity, provenance, and CI review findings | QSOBuilder | — | IN PROGRESS | The remediation lineage is consolidated on canonical PR #2 head; reachable submitted-state provenance, exact four-genome artifact-set assertion, explicit full immutable-protocol migration, and Aequitas invariant/reference validation are complete. Unambiguous digest semantics, declared dependencies, exact-head CI/clean-checkout replay, and review-thread disposition remain. |
...
PR #2 now uses the canonical remediation lineage. The submitted head includes Atlas plus a nine-artifact candidate manifest, deterministic canonicalization, immutable and Aequitas tests, and the earlier reported candidate set digest `4ed083cb204a77d1f1878aea8dbf9c61f996541c9b4de83c812bb461530d3eac`. The exact schema-bound genome directory is asserted as Atlas, Lyra, Nova, and Orion. Builder evidence adds explicit migration `QSO-GENOME-IMMUTABLE-ETHICS-MIGRATION-v1`, binding the authoritative full protocol digest `ecbab42031461161e91e511ce5f1ba19f1d2d75d81a8351a32f185b181c206af`, and a fail-closed Aequitas validator that checks canonical paths, source identifiers/schema version, exact references, duplicate IDs/paths before de-duplication, and every published invariant against the Sprite and all four genomes. Four exact-set tests, five migration tests, and eight Aequitas reference-integrity tests passed in focused replays. These remain candidate claims, not accepted capabilities: no exact-head workflow run is attached, and manifest identity semantics, dependency declaration, negative-fixture completeness, review-thread disposition, and downstream enforcement remain open.
...
Work only on the highest-priority unresolved acceptance finding. Do not add runtime behavior or new product scope while the candidate's manifest identity, dependency, and CI semantics remain unresolved.
...
- 2026-07-16 — QSOBuilder completed Aequitas invariant and reference integrity on implementation/test head `92dcc4b01c598248c72185b0352d8f4c8fdea9f2`. `scripts/validate_aequitas_binding.py` now rejects duplicate IDs/paths before de-duplication, non-canonical and escaping paths, missing/stale source identities and schema versions, incomplete reference sets, missing/unknown invariants, and any invariant that disagrees with Aequitas or the four genome source artifacts. Eight focused tests, command-line validator replay, and bytecode compilation passed under CPython 3.13.5; evidence is recorded in `reports/p0-aequitas-reference-integrity-validation.md`. P0 remains `IN PROGRESS`; the next unblocked finding is unambiguous compatibility-set digest semantics and binding of identity-bearing manifest metadata.
- 2026-07-16 — Consolidated the 15-commit remediation lineage onto existing PR #2 by fast-forwarding `builder/p0-four-genome-validation-20260716` from `5a435807487fd713c87465f3d23aaf9cd7cdd2b4` to `8c3d4ad3a8fc8cae864586d873cca319225c3e1d`. This preserves the original review record and eliminates the competing-head ambiguity. No acceptance or release promotion is implied; exact-head CI and remaining findings are still required.
