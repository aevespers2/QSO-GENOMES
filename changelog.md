...
- PR #2 remains the sole review path; reconciliation must integrate current `main` and every intended in-scope candidate commit without force-rewriting reviewed history unless the Architect explicitly approves and documents an alternative.
...
- Consolidation, reconciliation, scope containment, documentation synchronization, and candidate remediation are review corrections, not release promotions.
