"""QSIO integration boundary for QSO-GENOMES."""
...
class GenomeQSIOAdapter:
    repository = "qso-genomes"
    @staticmethod
    def enabled() -> bool:
        return os.getenv(FEATURE_FLAG, "false").lower() in {"1","true","yes","on"}
    def describe_local_entity(self, local_id: str) -> Mapping[str, Any]:
        return {"schema_version": SCHEMA_VERSION, "canonical_id": f"qso:genome:{local_id}", "local_id": local_id, "entity_type": "genome"}
    def to_qsi(self, local_event: object) -> Mapping[str, Any]:
        p = local_event if isinstance(local_event, Mapping) else vars(local_event)
        qsi = {"schema_version": SCHEMA_VERSION, "kind": "QSI", "source": self.repository, "action": p.get("action", "genome.revision"), "subject": p.get("canonical_id") or f"qso:genome:{p['local_id']}", "payload": dict(p)}
    ...
    def verify_compatibility(self) -> CompatibilityReport:
        return CompatibilityReport(True, unsupported_mappings=("runtime lifecycle is kernel-owned",))
